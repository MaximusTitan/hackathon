...
-- Create storage bucket for recruiting partner logos
INSERT INTO storage.buckets (id, name, public) VALUES 
    ('recruiting-logos', 'recruiting-logos', true)
ON CONFLICT DO NOTHING;

-- Create policy for recruiting logo uploads (admins only can upload)
CREATE POLICY "Admin can upload recruiting logos" ON storage.objects
    FOR INSERT WITH CHECK (
        bucket_id = 'recruiting-logos' AND
        EXISTS (
            SELECT 1 FROM auth.users
            WHERE auth.users.id = auth.uid()
            AND (
...
-- Create policy for recruiting logo access (anyone can view)
CREATE POLICY "Anyone can view recruiting logos" ON storage.objects
    FOR SELECT USING (bucket_id = 'recruiting-logos');

-- Create policy for admin to delete recruiting logos
CREATE POLICY "Admin can delete recruiting logos" ON storage.objects
    FOR DELETE USING (
        bucket_id = 'recruiting-logos' AND
        EXISTS (
            SELECT 1 FROM auth.users
