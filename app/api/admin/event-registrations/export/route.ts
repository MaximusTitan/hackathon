...
export async function GET(request: Request) {
  ...
  sales_presentation_link,
    ...
    const registrations = data.map(reg => {
      const userTestAttempts = testAttemptsMap.get(reg.id) || [];
      const latestAttempt = userTestAttempts.length > 0 
        ? userTestAttempts.reduce((latest: any, current: any) => 
            new Date(current.started_at) > new Date(latest.started_at) ? current : latest
          )
        : null;

      const screeningTest = reg.screening_test_id 
        ? screeningTestsMap.get(reg.screening_test_id) 
        : null;

      const awardAssignedByAdmin = reg.award_assigned_by 
        ? adminProfilesMap.get(reg.award_assigned_by) 
        : null;

      // Calculate score percentage and pass status
      let scorePercentage = null;
      let passed = false;
      if (latestAttempt && screeningTest) {
        scorePercentage = Math.round((latestAttempt.score / latestAttempt.total_questions) * 100);
        passed = scorePercentage >= (screeningTest.passing_score || 0);
      }
      ...
      return {
        ...
        screening_status: reg.screening_status || 'pending',
        presentation_status: reg.presentation_status || 'pending',
  ...
  sales_presentation_link: reg.sales_presentation_link,
        ...
        all_test_attempts: userTestAttempts,
